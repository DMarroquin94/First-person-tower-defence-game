...
Tween.prototype.lerp = function(t) {
	var index = 0;
	var number = this.times.length;
	while (index < number && t > this.times[index])
		index++;
	...
	var p = (t - this.times[index - 1]) / (this.times[index] - this.times[index - 1]);
	if (this.values[0] instanceof THREE.Vector3)
		return this.values[index - 1].clone().lerp(this.values[index], p);
	else //is a float
		return this.values[index - 1] + p * (this.values[index] - this.values[index - 1]);
}


function Particle() {
	this.position = new THREE.Vector3();
	this.velocity = new THREE.Vector3(); //unites per second
	this.acceleration = new THREE.Vector3();

	this.angle = 0;
	this.angleVeloctiy = 0;
	...
	this.age = 0;
	this.alive = 0; //use float insead of boolean for shader purposes
}
...
Particle.prototype.update = function(dt) {
	...
	this.angle += this.angleVeloctiy * 0.0175329251 * dt;
	this.angleVeloctiy += this.angleAcceleration * 0.0175329251 * dt;
